...
6. **Control socket** (`$RUNNER_TEMP/egress-filter-control.sock`) authenticates callers via `SO_PEERCRED` + process ancestry + `GITHUB_ACTION_REPOSITORY` match. Used by post.js for shutdown.
